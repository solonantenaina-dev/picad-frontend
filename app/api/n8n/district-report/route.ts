...
export async function POST(request: NextRequest) {
  ...
  if (contentType.includes("application/json")) {
    ...
    if (typeof body === "object" && body !== null && "name" in body) {
      districtName = String((body as { name?: unknown }).name ?? "").trim();
    }
  } else {
    districtName = (await request.text()).trim();
  }

  if (!districtName) {
    return NextResponse.json({ error: "name de district est requis" }, { status: 400 });
  }

  const url =
    process.env.N8N_WEBHOOK_URL ||
    "https://n8n.itdcmada.com/webhook/district";
